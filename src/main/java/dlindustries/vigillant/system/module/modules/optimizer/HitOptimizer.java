...
public final class HitOptimizer extends Module implements AttackListener, TickListener {
    ...
    public void onAttack(AttackEvent event) {
        ...
        // Cancel if holding a weapon
        if (isWeapon(currentItem)) return;

        // Cancel switch-back if currently waiting to switch back
        if (shouldSwitchBack) {
            switchTimer = 0;  // Reset delay timer
            return;
        }

        // Find sword slot if required
        if (requireSword.getValue()) {
            int swordSlot = findSwordSlot();
            if (swordSlot == -1) return;

            // Save original slot if we're going to switch back
            ...
            // Prepare to switch back if enabled
